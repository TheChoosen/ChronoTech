-- Migration pour ajouter les tables manquantes du Customer 360
-- Exécuter ce script pour créer les tables nécessaires aux nouvelles fonctionnalités
...
-- Index pour les recherches de documents
CREATE INDEX `idx_customer_documents_created` ON `customer_documents` (`created_at`);
CREATE INDEX `idx_customer_documents_filename` ON `customer_documents` (`original_filename`);

-- Vérifier si la table customer_activity existe, sinon la créer
...
-- Ajout de quelques activités de test si la table est vide
INSERT IGNORE INTO `customer_activity` (`customer_id`, `activity_type`, `title`, `description`, `created_at`) VALUES
...
-- Note: Cette partie sera gérée par le code Python lors du premier upload

-- Affichage du résultat
SELECT 'Tables customer_documents et customer_activity créées avec succès' as status;
SELECT COUNT(*) as documents_count FROM customer_documents;
SELECT COUNT(*) as activities_count FROM customer_activity;
